...
(sleep 5; pkill main) &  # Kill after opening  since open 
open Paddle\ Battle.app  # Open the regular way.
[[ 0 == $? ]] && echo "Success" || { echo "App failed to open!"; exit 1; }  # Make sure it opened correctly.
sleep 10  # wait til Paddle Battle is killed fully. (`open` doesn't block)

# Test running the code
(sleep 5; pkill -7 main) &  # Kill with a specific, uncommon signal.
